...
import { useAuth } from '../../context/AuthContext'; // Import context yang sudah dibuat
...
const Login = () => {
    ...
    // State untuk menangani loading dan error dari Laravel
    const [errors, setErrors] = useState({});
    const [isSubmitting, setIsSubmitting] = useState(false);

    // Panggil fungsi login dari AuthContext dan navigasi dari React Router
    const { login } = useAuth();
    ...
    const handleSubmit = async (e) => {
        ...
        setErrors({}); // Kosongkan error sebelumnya

        // Panggil API ke Laravel
        const success = await login({ email, password, setErrors });
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-primary/5 px-4 font-poppins">
            <div className="max-w-md w-full bg-universal rounded-3xl shadow-xl p-8 border border-gray-100">
                ...
                <div className="text-center mb-8">
                    ...
                    <h2 className="text-2xl font-bold text-dark mt-4">Selamat Datang Kembali!</h2>
                    <p className="text-dark/60 text-sm mt-2">Silakan login untuk melanjutkan booking.</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            required
                        />
                        {/* Tampilkan pesan error email dari Laravel jika ada */}
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-1">
                            <label className="block text-sm font-medium text-dark/80">Password</label>
                            <a href="#" className="text-xs font-semibold text-primary hover:text-secondary transition-colors">Lupa Password?</a>
                        </div>
                        <input
                            type="password"
                            value={password}
                    ...
                    <button
                        ...
                        className={`w-full font-bold py-3.5 rounded-xl shadow-md transition-all duration-300 mt-2 flex justify-center items-center ${isSubmitting ? 'bg-primary/70 text-universal cursor-not-allowed' : 'bg-primary text-universal hover:bg-opacity-90 hover:-translate-y-0.5'
                            }`}
                    >
                        {isSubmitting ? (
                ...
                <div className="flex items-center my-6">
                    ...
                    <span className="px-4 text-xs text-dark/40 font-medium uppercase">Atau masuk dengan</span>
                    <div className="grow border-t border-gray-200"></div>
                </div>

                {/* Tombol Login Google (Bisa disiapkan logikanya nanti setelah setup Laravel Socialite) */}
                <button
                    type="button"
                ...
                <p className="text-center text-sm text-dark/70 mt-8">
                    Belum punya akun? <Link to="/register" className="text-primary font-bold hover:text-secondary transition-colors">Daftar sekarang</Link>
                </p>
